...
class Client:
	...
	async def handle_packet(self, connection: Connection, packet: Packet) -> bool:
		...
		if CCC == (6, 20): # on_time CUSTOM
			for _ in range(0, 15):
				packet.read8() # Unused data

			units = []

			for _ in range(0, 4):
				if packet.read16() == 2: # checks if two digits
					units.append(packet.readBytes().decode() + packet.readBytes().decode())
				else:
					units.append(packet.readBytes().decode())

			self.dispatch('time', f"I have {units[0]} day(s), {units[1]} hour(s), {units[2]} minute(s) and {units[3]} second(s) play time.")
		
		elif CCC == (5, 2): # on_map_load CUSTOM
			code = packet.read32()
			...
			for _ in range(0, packet.read16()): # xml data
				packet.read8()

			author = packet.readUTF()
			
			category = packet.read8()
			is_mirror = packet.readBool()
			...
			self.dispatch('map_load', map_data)

		elif CCC == (20, 3): # on_sale CUSTOM
			if not packet.readBool(): # Is not sales item return
				return

			is_shaman = not packet.readBool()

			item_id = packet.read32()

			part1 = item_id
			part2 = 0
			if item_id > 9999:
				part1 = int((item_id - 10000) / 10000)
				part2 = int((item_id - 10000) % 10000)
			elif item_id > 99:
				part1 = int(item_id / 100)
				part2 = int(item_id % 100)

			packet.readBool() # ?
			expire_time = packet.read32()
			...
			# CUSTOM
			elif TC == 91: # new tribe member
				self.dispatch('new_member', packet.readUTF())

			elif TC == 92: # tribe member left
				self.dispatch('left_member', packet.readUTF())

			elif TC == 93: # tribe member kicked
				self.dispatch('kicked_member', packet.readUTF())
