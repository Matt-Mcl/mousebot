...
async def get_ip():
	...
	try:
		async with aiohttp.ClientSession() as session:
			async with session.get(url, headers=headers, timeout=10) as resp: # CUSTOM TIMEOUT
		...
		print(f"CAUGHT ERROR IN GET CFM GET IP (no Message after colon means timeout): {e}")
		data = dict()
